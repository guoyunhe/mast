"""Hosts tab component for the SSH module."""
...
from yast3.modules.ssh.dialogs import SSHEditDialog
from yast3.modules.ssh.ssh import SSHConfigEntry, load_ssh_config, save_ssh_config
...
class HostsTab(QWidget):
    ...
    def load_config(self) -> None:
        ...
        try:
            self.entries = load_ssh_config()
        except FileNotFoundError:
            # Config file doesn't exist - that's OK
            pass
        except PermissionError:
            QMessageBox.warning(self, _("Error"), _("Cannot read SSH config. Check permissions."))
            return
        except Exception as e:
            QMessageBox.warning(self, _("Error"), _("Failed to load SSH config: {0}").format(str(e)))
            return
    ...
    def add_host(self) -> None:
        ...
        if dialog.exec():
            ...
            if host:
                self.entries.append(SSHConfigEntry(
                    enabled=True,
                    host=host,
                    options=options,
                    editable=True
    ...
    def edit_host(self) -> None:
        ...
        if dialog.exec():
            ...
            if new_host:
                self.entries[current_row] = SSHConfigEntry(
    ...
    def save_config(self) -> None:
        ...
        result = save_ssh_config(self.entries)
